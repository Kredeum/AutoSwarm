...
import { UNDEFINED_ADDRESS, UNDEFINED_DATA, DIVISION_BY_ZERO, ONE_WEEK, ONE_DAY, ONE_HOUR, ONE_MONTH, ONE_YEAR, BZZ_DECIMALS, BUCKET_DEPTH } from "../constants/constants";
...
const displayBalance = (balance: bigint | undefined, decimals: number = 18, toFixed: number = 3): string => {
...
const displayLink = (url: URL | string | undefined): string => (url === undefined ? "" : `<a href="${url}" target="_blank">${utilsTruncate(url.toString())}</a>`);

const displayNftLink = (chainId: number | undefined, collection: string | undefined, tokenId: bigint | undefined): string => {
  if (!(chainId && chainId > 0 && collection && isAddress(collection) && tokenId && tokenId >= 0)) return UNDEFINED_DATA;

  const url = `https://app.kredeum.com/#/${chainId}/${collection}/${tokenId}`;
  return `<a href="${url}" target="_blank">#${tokenId}</a>`;
};

const displaySizeBytes = (size: bigint | number | undefined): string => `${size?.toString() || UNDEFINED_DATA} bytes`;
