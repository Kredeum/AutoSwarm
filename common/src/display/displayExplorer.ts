...
import { UNDEFINED_ADDRESS } from "../constants/constants";
...
const displayExplorerNft = (chainId: number | undefined, collection: string | undefined, tokenId: bigint | undefined): string => {
  if (!(chainId && collection && tokenId && tokenId >= 0)) return "";
  const explorer = chainGetExplorer(chainId);
  if (!explorer) return `#${chainId}`;

  return `<a href="${explorer}/nft/${collection}/${tokenId}" target="_blank">#${utilsTruncate(tokenId.toString())}</a>`;
};

const _displayExplorerAddress = (chainId: number | undefined, addr: Address | undefined): string => {
  if (!chainId) return "";
...
const displayExplorerAddress = (chainId: number | undefined, addr: Address | undefined): string => {
  ...
  return chainId && addr && isAddress(addr) ? _displayExplorerAddress(chainId, addr) : UNDEFINED_ADDRESS;
};
const displayExplorerField = (chainId: number, field: string): string => displayExplorerAddress(chainId, addressesGetField(chainId, field));
