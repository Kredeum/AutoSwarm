...
test('stampBzzToNBal should convert BZZ to NBal', () => {
	...
	expect(stampBzzToNBal(1_000n, 1)).toBe(1_000n * BUCKET_SIZE);
	expect(stampBzzToNBal(1_000n, 4)).toBe(250n * BUCKET_SIZE);

	expect(stampBzzToNBal(1_000n, 10)).toBe(100n * BUCKET_SIZE);
	expect(stampBzzToNBal(1_000n, 10)).toBe(409600n);

	expect(stampBzzToNBal(1_000n * 10n ** 9n, 20)).toBe(50n * 10n ** 9n * BUCKET_SIZE);
	expect(stampBzzToNBal(1_000n * 10n ** 9n, 20)).toBe(204_800_000_000_000n);

	// check integer division is done the right way
	expect(stampBzzToNBal(10n ** 16n, 24)).not.toBe((10n ** 16n / 24n) * BUCKET_SIZE);
	expect(stampBzzToNBal(10n ** 16n, 24)).not.toBe(10n ** 16n * (BUCKET_SIZE / 24n));
	expect(stampBzzToNBal(10n ** 16n, 24)).toBe((10n ** 16n * BUCKET_SIZE) / 24n);
	expect(stampBzzToNBal(10n ** 16n, 24)).toBe(1_706_666_666_666_666_666n);

	// with no rounding problems, shortcuts are allowed
	expect(stampBzzToNBal(10n ** 12n, 20)).toBe((1_000n / 20n) * 10n ** 9n * BUCKET_SIZE);
	expect(stampBzzToNBal(10n ** 12n, 20)).toBe(204_800_000_000_000n);
});

test('stampNBalToBzz should convert NBal to BZZ', () => {
	expect(stampNBalToBzz(1_000n, 2)).toBe(2000n / BUCKET_SIZE);
	expect(stampNBalToBzz(24_000_000n, 2)).toBe(48_000_000n / BUCKET_SIZE);
