...
import {Stamp, Batch, IAutoSwarmMarketV1} from "../interfaces/IAutoSwarmMarketV1.sol";
import {IPostageStampLegacy} from "../interfaces/IPostageStampLegacy.sol";

import {console} from "forge-std/console.sol";

contract AutoSwarmMarketV1 is IAutoSwarmMarketV1, Ownable {
    ...
    function setCurrentYear(uint256 year) external override(IAutoSwarmMarketV1) onlyOwner {
        currentYear = year;
    }

    function setPostage(address postageStamp_) external override(IAutoSwarmMarketV1) onlyOwner {
        _setPostage(postageStamp_);
    }

    function buyBatch(uint256 year) external override(IAutoSwarmMarketV1) onlyOwner returns (bytes32 batchId) {
    ...
    function extendsBatch(uint256 year, uint8 deltaDepth) external override(IAutoSwarmMarketV1) onlyOwner {
    ...
    function getStampPrice(uint256 year, uint256 size) public view override(IAutoSwarmMarketV1) returns (uint256 bzz) {
        uint256 ttl = getBatchTtl(year);
    ...
    function withdraw(address token) external override(IAutoSwarmMarketV1) onlyOwner {
        if (token == address(0)) {
    ...
    function topUpBatch(uint256 year, uint256 ttl) external override(IAutoSwarmMarketV1) onlyOwner {
        _topUpBatch(year, ttl);

        emit UpdateBatch(year, getBatchDepth(year), ttl);
    }

    function diluteBatch(uint256 year, uint8 deltaDepth) external override(IAutoSwarmMarketV1) onlyOwner {
        _diluteBatch(year, deltaDepth);

        emit UpdateBatch(year, getBatchDepth(year) + deltaDepth, getBatchTtl(year));
    }

    // function getOneYearAmount(uint256 year) public view override(IAutoSwarmMarketV1) returns (uint256) {}

    function getBatchSizeLimit(uint256 year) public view override(IAutoSwarmMarketV1) returns (uint256) {
        return (BUCKET_SIZE << getBatchDepth(year)) / RATIO;
    }

    function getBatchDepth(uint256 year) public view override(IAutoSwarmMarketV1) returns (uint8) {
        return postageStamp.batchDepth(batch[year].batchId);
    }

    function getBatchTtl(uint256 year) public view override(IAutoSwarmMarketV1) returns (uint256) {
